...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ call: call || callResponse, retell_response: callResponse }, { status: 201 });
  } catch (error) {
    console.error('Error creating call:', error);
    const errorMessage = error instanceof Error ? error.message : 'Error creating call';
